...
private:
    ...
    bool Erase_Block(uint32_t offset, uint32_t length)
    {
        ...
        if (m_cmd_type == DSTT_CMD_TYPE_1) {
            ...
            dstt_flash_command(0x87, 0x2AAA, 0x55);
            dstt_flash_command(0x87, 0x5555, 0x90);
            uint8_t writeProtected = (uint8_t)(dstt_flash_command(0, address | 2, 0));
            dstt_reset();

            if (writeProtected) {
                logMessage(LOG_ERR, "DSTT: Tried to write to write-protected sector %d!", address >> 14);
                return false;
            }
...
public:
    ...
    bool writeFlash(uint32_t address, uint32_t length, const uint8_t *buffer)
    {
        ...
        for (auto const& block_sz: erase_blocks) {
            ...
            if (address < block_end) {
                ...
                uint32_t copy_len = min(src_len, dest_len);
    ...
    bool injectNtrBoot(uint8_t *blowfish_key, uint8_t *firm, uint32_t firm_size) {
        ...
        writeFlash(0x1000, blowfish_key, 0x48);
        writeFlash(0x2000, blowfish_key + 0x48, 0x1000);
        writeFlash(0x7E00, firm, firm_size);
